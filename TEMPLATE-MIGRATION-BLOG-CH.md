...
- **⚠️ Attacher le domaine au projet Pages** (sinon HTTP 522 après bascule) : via l'API
  `POST /accounts/<account>/pages/projects/<projet>/domains {"name": "domaine.ch"}` (+ `www.domaine.ch`) —
  le CNAME DNS seul ne suffit pas, Pages ne sert que les Hosts attachés au projet
